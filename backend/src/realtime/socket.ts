  ...
  io.use(async (socket: UserSocket, next) => {
    ...
    */
    ...
    try {
      ...
      console.log('[Socket Auth Middleware] Received token from:', 
                 authToken ? 'auth object' : (queryToken ? 'query string' : 'nowhere'), 
                 token ? `Token received (${typeof token})` : 'No token');
      ...
      try {
        ...
        decoded = jwt.verify(tokenStr, process.env.JWT_SECRET || 'user-secret-key');
