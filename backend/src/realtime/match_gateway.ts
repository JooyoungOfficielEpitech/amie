...
export class MatchGateway {
  ...
  private readonly creditService = new CreditService(); // 추가: 크레딧 서비스 인스턴스 생성
  ...
  private async initialize() {
    ...
    matchNamespace.use(async (socket: UserSocket, next) => {
    ...
    matchNamespace.on('connection', async (socket: UserSocket) => {
  ...
  // --- Start Match Handler ---
  private async handleStartMatch(socket: UserSocket) {
    try {
  ...
  */
  ...
  private async handleCancelMatch(socket: UserSocket) {
    try {
      if (!socket.userId) return; // No user ID, nothing to cancel
      const userId = socket.userId;
  ...
  private async handleCheckMatchStatus(socket: UserSocket) {
    try {
      ...
      const blurredProfileImages = matchedUser.profileImages.map((img: string) => `blurred-${img}`);
  ...
  private async processMatching() {
    ...
    // 성별별 대기 목록
    const maleWaiting = waitingUsers.filter((user: IMatchQueue) => user.gender === 'male');
    const femaleWaiting = waitingUsers.filter((user: IMatchQueue) => user.gender === 'female');
  ...
  private async tryMatchForUser(userId: string, gender: string) {
    ...
    for (const [otherUserId, otherSocket] of this.matchingUsers.entries()) {
        if (otherUserId !== userId && otherSocket.userInfo?.gender === targetGender) {
            ...
            const potentialMatchUser = await User.findById(otherUserId);
  ...
  private async matchUsers(maleUserId: string, femaleUserId: string) {
    try {
      ...
      await User.updateMany(
        { _id: { $in: [maleUserId, femaleUserId] } },
      ...
      if (maleUser && femaleUser) {
        // 남성 사용자에게 알림
        if (maleSocket) {
          const blurredFemaleImages = femaleUser.profileImages.map((img: string) => `blurred-${img}`);
        ...
        // 여성 사용자에게 알림
        if (femaleSocket) {
          const blurredMaleImages = maleUser.profileImages.map((img: string) => `blurred-${img}`);
      ...
      const maleSocket = this.matchingUsers.get(maleUserId);
      const femaleSocket = this.matchingUsers.get(femaleUserId);
      if (maleSocket) maleSocket.emit('match_error', { message: '매칭 처리 중 오류 발생' });
      if (femaleSocket) femaleSocket.emit('match_error', { message: '매칭 처리 중 오류 발생' });
