...
exports.getQueueStats = exports.findMatch = exports.getAllQueueEntries = exports.checkQueueStatus = exports.leaveQueue = exports.joinQueue = void 0;
exports.syncUserToRedis = syncUserToRedis;
exports.syncUserToMongodb = syncUserToMongodb;
...
async function syncQueues() {
    try {
        const redisClient = (0, redis_service_1.getRedisClient)();
        ...
        const mongodbWaitingUsers = await MatchQueue_1.default.find({ isWaiting: true });
        ...
        for (const userId of [...redisMaleOnlyUsers, ...redisFemaleOnlyUsers]) {
            ...
            if (user) {
                ...
                const existingEntry = await MatchQueue_1.default.findOne({
                    userId,
                    isWaiting: true
                });
                if (!existingEntry) {
                    ...
                    logger_1.logger.info(`대기열 동기화: Redis 사용자 ${userId}를 MongoDB에 추가함`);
                }
            }
            else {
                ...
                logger_1.logger.warn(`대기열 동기화: 존재하지 않는 사용자 ${userId}를 Redis에서 제거함`);
            }
        }
        ...
        for (const userId of mongodbMaleOnlyUsers) {
            ...
            syncCount++;
            logger_1.logger.info(`대기열 동기화: MongoDB 남성 사용자 ${userId}를 Redis에 추가함`);
        }
        for (const userId of mongodbFemaleOnlyUsers) {
            await redisClient.zadd('match:queue:female', Date.now(), userId);
            syncCount++;
            logger_1.logger.info(`대기열 동기화: MongoDB 여성 사용자 ${userId}를 Redis에 추가함`);
        }
        return {
            success: true,
            syncCount,
        ...
        logger_1.logger.error('대기열 동기화 중 오류:', error);
        return {
            success: false,
            error: error.message
        };
...
const getQueueStats = async (req, res) => {
    try {
        ...
        const maleCount = await MatchQueue_1.default.countDocuments({
            gender: 'male',
            isWaiting: true
        });
        const femaleCount = await MatchQueue_1.default.countDocuments({
            gender: 'female',
            isWaiting: true
        });
        // 대기 중인 남성/여성 사용자 정보 가져오기
        const maleUsers = await MatchQueue_1.default.find({
            gender: 'male',
            isWaiting: true
        }).populate('userId', 'nickname _id');
        const femaleUsers = await MatchQueue_1.default.find({
            gender: 'female',
            isWaiting: true
        }).populate('userId', 'nickname _id');
        ...
        const maleUserDetails = maleUsers.map(entry => {
            ...
            if (entry.userId && typeof entry.userId === 'object' && 'nickname' in entry.userId) {
                return {
                    ...
                    nickname: entry.userId.nickname,
        ...
        try {
            ...
            const redisOnlyMaleUsers = redisMaleUsers.filter(id => !maleUserDetails.some(user => user.userId === id));
            const redisOnlyFemaleUsers = redisFemaleUsers.filter(id => !femaleUserDetails.some(user => user.userId === id));
            // Redis에만 있는 사용자 정보 가져오기
            if (redisOnlyMaleUsers.length > 0 || redisOnlyFemaleUsers.length > 0) {
                logger_1.logger.info(`Redis에만 존재하는 대기 사용자: 남성=${redisOnlyMaleUsers.length}, 여성=${redisOnlyFemaleUsers.length}`);
                // Redis에만 있는 모든 사용자 ID 목록
                const redisOnlyUserIds = [...redisOnlyMaleUsers, ...redisOnlyFemaleUsers];
                // MongoDB에서 해당 사용자 정보 조회
                const redisOnlyUsers = await User_1.default.find({
...
async function syncUserToRedis(userId, gender) {
    try {
        const queueName = gender === 'male' ? 'match:queue:male' : 'match:queue:female';
        await (0, redis_service_1.addToQueue)(queueName, userId);
        logger_1.logger.info(`사용자 ${userId} 동기화: MongoDB → Redis`);
        return true;
    }
    catch (error) {
        logger_1.logger.error(`사용자 ${userId} Redis 동기화 실패:`, error);
        return false;
    }
}
// 단일 사용자를 Redis에서 MongoDB로 동기화
async function syncUserToMongodb(userId) {
    try {
        // 사용자 정보 확인
        const user = await User_1.default.findById(userId);
        if (!user) {
            logger_1.logger.error(`Redis 동기화 오류: 사용자 ${userId}를 찾을 수 없음`);
            return false;
        }
        // 이미 대기 중인지 확인
        const existingEntry = await MatchQueue_1.default.findOne({
        ...
        if (existingEntry) {
            logger_1.logger.info(`사용자 ${userId}는 이미 MongoDB 대기열에 있음`);
            return true;
        }
        // 새 대기열 항목 생성
        const queueEntry = new MatchQueue_1.default({
        ...
        // User 모델 업데이트
        await User_1.default.findByIdAndUpdate(userId, { isWaitingForMatch: true });
        logger_1.logger.info(`사용자 ${userId} 동기화: Redis → MongoDB`);
        return true;
    }
    catch (error) {
        logger_1.logger.error(`사용자 ${userId} MongoDB 동기화 실패:`, error);
        return false;
    }
}
